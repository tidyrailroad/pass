...
RUN \
    ...
    rm -rf ${WORK_DIR} && \
    apk del git make && \
    echo user ALL = NOPASSWD: /usr/local/sbin/git.sh > /etc/sudoers.d/git && \
    ...
    echo -en '#!/bin/sh\n\ndocker run --interactive --rm --volume /var/run/docker.sock:/var/run/docker.sock:ro --volume ${DOT_GNUPG}:/home/.gnupg --volume ${DOT_PASSWORD_STORE}:/usr/local/src --workdir /usr/local/src --user user emorymerryman/gpg:0.1.0' > /root/bin/gpg.sh' && \
    chmod 0500 /usr/local/bin/git.sh && \
    ...
    echo -en '#!/bin/sh\n\ndocker run --interactive --rm --volume /var/run/docker.sock:/var/run/docker.sock:ro --volume ${DOT_PASSWORD_STORE}:/usr/local/src --workdir /usr/local/src --user user emorymerryman/tree:0.1.0' > /root/bin/tree.sh' && \
    chmod 0500 /usr/local/bin/tree.sh && \
    true
